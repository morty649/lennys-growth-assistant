...
The product should feel like a calm research instrument rather than a generic chatbot. It keeps the editorial typography, restrained monochrome/cream palette, thin rules, and terminal-like identity inspired by Physical Intelligence while borrowing the useful interaction pattern of Claude's contextual artifact workspace.

The design does not imitate either product pixel for pixel. Its goal is to make evidence, model state, and generated work understandable without making users learn the underlying infrastructure.
