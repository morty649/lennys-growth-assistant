...
localhost browser
  ...
  -> Pi agent harness
     -> explicitly selected Qwen/Ollama or Claude backend
...
For Ollama, Pi injects `reasoning_effort: none` into every request and verifies its transport hook ran. The selected local tag is `qwen3:8b`; the previously qualified 14B tag is a fallback only if the smaller model fails the release gates. Claude uses the same Pi tool registry when explicitly configured. Groq is disabled in the v0.2 product path.
...
- PostgreSQL full-text search and Ollama `nomic-embed-text` dense search produce independent candidates.
...
- No provider fallback is automatic. Cloud use requires explicit Claude selection.
...
Anthropic is an outbound, explicitly selected inference dependency rather than a hosted-application requirement. Local PostgreSQL remains the v0.2 default; Supabase/Railway and public web hosting are deferred.
